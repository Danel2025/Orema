  ...
  try {
    ...
    if (process.env.NODE_ENV !== "production") {
      console.log(
        ...
        usersToCreate.map((u) => ({ email: u.email, role: u.role, pin: "****" }))
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message:
        "Configuration Supabase Auth terminee. Consultez les logs serveur pour les credentials.",
    });
  } catch (error) {
