...
import { RefreshCw, Check, X, Clock, AlertTriangle, CloudOff } from "lucide-react";
...
interface SyncStatusProps {
  ...
  isSyncing: boolean;
  /** true si le réseau est hors ligne */
  isOffline: boolean;
  /** Mutations en attente groupées par type */
  ...
  /** Historique des dernières syncs */
  syncHistory: SyncHistoryEntry[];
  /** Conflits non résolus */
  conflicts: SyncConflict[];
  /** Callback pour déclencher une sync manuelle */
  onSyncNow: () => void;
  /** Trigger element (le bouton/icône qui ouvre le popover) */
        ...
        return (
          <Flex key={entry.id} align="center" gap="2">
            ...
            <Text size="1" style={{ color: "var(--gray-11)" }}>
              {entry.itemCount} élément{entry.itemCount > 1 ? "s" : ""}
            </Text>
          </Flex>
        );
      })}
    ...
    <>
      ...
      <Box>
        <Flex align="center" gap="2" mb="2">
          <AlertTriangle size={14} style={{ color: "var(--red-9)" }} aria-hidden="true" />
          <Text size="2" weight="bold" style={{ color: "var(--red-9)" }}>
            {conflicts.length} conflit{conflicts.length > 1 ? "s" : ""} non résolu
...
export function SyncStatus({
  ...
  return (
    <Popover.Root>
      ...
      <Popover.Content size="2" maxWidth="340px" style={{ maxHeight: 460, overflowY: "auto" }}>
        <Flex direction="column" gap="3">
          ...
          <Box>
            <Flex align="center" gap="2" mb="2">
              <Clock size={14} style={{ color: "var(--gray-9)" }} aria-hidden="true" />
