...
/**
 * Composant d'affichage et d'édition des permissions par rôle
 *
 * Affiche les 47 permissions granulaires organisées par groupes
 * avec des switches éditables pour les admins.
 */

import { useState, useEffect, useTransition } from "react";
import { Dialog, Button, Flex, Text, Badge, Box, Switch, Callout, Spinner } from "@radix-ui/themes";
import { ScrollArea } from "@/components/ui";
import * as Accordion from "@radix-ui/react-accordion";
import { Shield, Check, X, CaretDown, ArrowsClockwise, Info } from "@phosphor-icons/react";
...
export function RolePermissionsModal({ open, onOpenChange, selectedRole }: RolePermissionsProps) {
  const [activeTab, setActiveTab] = useState<Role>(selectedRole || "MANAGER");
  const [permissionConfigs, setPermissionConfigs] = useState<Record<Role, PermissionConfig> | null>(
    null
  );
    ...
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <Dialog.Description size="2" mb="4">
          Configurez les permissions associées à chaque rôle dans le système
        </Dialog.Description>
    ...
    <Flex direction="column" gap="3">
      ...
      {isCustom ? (
        <Callout.Root color="amber" size="1">
          ...
          <Callout.Text>
            Ce rôle a des permissions personnalisées.
            {isEditable ? (
              <Button
                variant="ghost"
                size="1"
      ...
      {role === "SUPER_ADMIN" && (
        <Callout.Root color="blue" size="1">
          ...
          <Callout.Text>Les permissions de Super Admin ne peuvent pas être modifiées.</Callout.Text>
        </Callout.Root>
      )}
            ...
            >
              <Accordion.Header>
                ...
                >
                  ...
                  <CaretDown size={16} style={{ color: "var(--gray-9)" }} />
                </Accordion.Trigger>
              </Accordion.Header>
...
function getRoleDescription(role: Role): string {
  switch (role) {
    case "SUPER_ADMIN":
      return "Accès complet à toutes les fonctionnalités du système. Réservé au propriétaire ou gestionnaire principal.";
    case "ADMIN":
      return "Gestion complète de l'établissement : employés, produits, rapports et paramètres.";
    case "MANAGER":
      return "Gestion opérationnelle : produits, stock, clients. Peut faire des ventes et appliquer des remises.";
    case "CAISSIER":
      return "Opérations de caisse : ventes, encaissements, gestion des clients. Accès limité aux rapports.";
    case "SERVEUR":
...
/**
 * Composant compact pour afficher les permissions d'un rôle
 */
