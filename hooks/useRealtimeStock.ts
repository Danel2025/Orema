...
export function useRealtimeStock({
  ...
  const handleStockUpdate = useCallback((produit: Produit) => {
    if (!produit.gerer_stock || produit.stock_actuel === null || produit.stock_min === null) {
      setLowStockAlerts((prev) => prev.filter((a) => a.produitId !== produit.id));
      return;
    }
  ...
  const dismissAlert = useCallback((produitId: string) => {
    setLowStockAlerts((prev) => prev.filter((a) => a.produitId !== produitId));
  }, []);
  ...
  useEffect(() => {
    ...
    const cleanup = subscribeToStock(supabase, etablissementId, {
