...
import { detectConflict, createConflictRecord, type ConflictRecord } from "./conflict-resolver";
...
export class SyncEngine {
   ...
   */
  private async syncOne(mutation: MutationEntry): Promise<"synced" | "failed" | "conflict"> {
    ...
    try {
      const result = await this.executor(mutation.entity, mutation.type, mutation.data);
      ...
      await markFailed(this.db, mutation.id, result.error || "Unknown server error");
      return "failed";
    } catch (error) {
  ...
  private async handleConflict(
    ...
    const localUpdatedAt = (mutationData.updated_at as string) || new Date().toISOString();
    const remoteData = (serverResult.data as Record<string, unknown>) || {};
    const remoteUpdatedAt = (remoteData.updated_at as string) || new Date().toISOString();

    // Verifier si c'est un vrai conflit via les timestamps
    const lastSynced = this._lastSyncAt || 0;
    const isConflict = detectConflict(localUpdatedAt, remoteUpdatedAt, lastSynced);
